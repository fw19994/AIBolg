...
export default function EditorEditPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      navigate(`/login?redirect=${encodeURIComponent(`/editor/${id}`)}`, { replace: true });
    }
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) => {
        ...
        setTagIds(Array.isArray(a.tags) ? a.tags.map((t) => t.id) : []);
      })
  ...
  const handleSave = () => {
    ...
    api
      ...
      .then(() => {
        api.getArticle(id).then((r) => setArticle(r.data));
      })
      .catch((e) => {
    ...
    return (
      <div className="min-h-screen w-full flex bg-slate-50 dark:bg-slate-950">
        ...
        <main className="flex-1 min-w-0 px-0 flex flex-col items-center justify-center gap-4 text-slate-500 dark:text-slate-400">
          <p>文章不存在或已删除</p>
          <Link to="/editor" className="text-emerald-600 hover:underline dark:text-emerald-400">写新文章</Link>
        </main>
      </div>
    );
  }

  const savedAt = article.updated_at ? new Date(article.updated_at).toLocaleString('zh-CN') : '';
  ...
  return (
    <div className="min-h-screen w-full flex overflow-hidden bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      <aside className="hidden md:flex md:flex-col w-60 border-r border-slate-200/90 bg-white/85 px-4 py-4 gap-4 text-xs dark:border-slate-800/80 dark:bg-slate-900/70">
        ...
        <div className="space-y-2 overflow-y-auto pr-1 flex-1 min-h-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <Link
            to="/articles"
          ...
          {drafts.map((d) => (
            <Link
              key={d.id}
              to={`/editor/${d.id}`}
              className={`w-full flex flex-col items-start gap-1 px-3 py-2 rounded-xl text-left block ${
                d.id === id
                  ? 'bg-slate-200/90 text-slate-900 dark:bg-slate-800/90 dark:text-slate-100'
                  : 'hover:bg-slate-200/80 text-slate-800 dark:hover:bg-slate-800/70 dark:text-slate-200'
              }`}
            >
              <div className="flex items-center justify-between w-full">
                <span className="text-xs font-medium line-clamp-1">{d.title || '无标题'}</span>
                <span className={`text-[10px] shrink-0 flex items-center gap-1 ${d.id === id ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-500 dark:text-slate-400'}`}>
                  {d.id === id && <i className="fa-solid fa-cloud" />}
                  <span>{d.id === id ? '已同步' : '草稿'}</span>
                </span>
              </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 px-0">
        <header className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-slate-200/90 bg-white/80 backdrop-blur-sm shrink-0 dark:border-slate-800/80 dark:bg-slate-900/70">
          <div className="flex items-center gap-3 min-w-0">
            <div className="min-w-0">
              <input
                ...
                className="bg-transparent focus:outline-none text-sm md:text-base font-medium text-slate-900 w-full max-w-md dark:text-slate-50"
              />
              <p className="text-[11px] text-slate-400 flex items-center gap-2 mt-0.5">
                <span>自动草稿已保存 · {savedAt}</span>
          ...
          <div className="flex items-center gap-2 md:gap-3 text-xs shrink-0">
            ...
            {article.status === 'published' && (
              <Link
                to={`/article/${id}`}
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-emerald-500 text-slate-950 font-medium hover:bg-emerald-400"
              >
        ...
        <section className="flex-1 flex overflow-hidden min-h-0" ref={editorWrapRef} onMouseUp={handleEditorMouseUp}>
          <div className="flex-1 flex flex-col min-w-0 px-3 md:px-6 py-3 md:py-4 min-h-0">
            <RichMarkdownEditor
              ...
              height={520}
              placeholder="开始写作… 支持 Markdown 语法，左侧编辑、右侧实时预览"
        ...
        <AIAssistantDialog
          ...
          articleId={Number.isFinite(id) ? id : undefined}
        />
      </main>
    </div>
  );
